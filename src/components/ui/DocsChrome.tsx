...
const TOP = [
  ...
  { id: "brandGuidelines", label: "Marca" },
];

const SIDE = [
  { id: "documentsHub", label: "Visão geral" },
  { id: "adminDocLibrary", label: "Biblioteca (admin)" },
  { id: "contractDownloads", label: "Contratos" },
  { id: "ipDocumentation", label: "Documentação de PI" },
  { id: "ipDrawings", label: "Desenhos de PI" },
  { id: "patentClaims", label: "Reivindicações" },
  { id: "gridSystemPdf", label: "Sistema de grelha" },
  { id: "brandGuidelines", label: "Manual da marca" },
];

/** Web frame for the docs / IP section (1180×760): browser chrome + top nav +
 *  left document sidebar. `topActive` highlights the nav; `side` the sidebar. */
export function DocsChrome({ url, topActive, side, children }: { url: string; topActive: string; side: string; children: ReactNode }) {
  ...
  return (
    <div style={{ width: 1180, height: 760, borderRadius: 14, background: "#F8F5F0", overflow: "hidden", boxShadow: "0 40px 70px -34px rgba(28,24,21,.45), 0 0 0 1px rgba(28,24,21,.05)", display: "flex", flexDirection: "column" }}>
      <div style={{ height: 46, flex: "none", background: "#EDE6DA", borderBottom: "1px solid #DDD3C4", display: "flex", alignItems: "center", gap: 8, padding: "0 16px" }}>
        <div style={{ display: "flex", gap: 7 }}>{["#E0655A", "#E6B95A", "#6FBE7A"].map((c) => (<div key={c} style={{ width: 12, height: 12, borderRadius: "50%", background: c }} />))}</div>
      ...
      <div style={{ height: 64, flex: "none", background: "#1A1814", display: "flex", alignItems: "center", padding: "0 30px", gap: 26 }}>
        <button onClick={() => navigate("/landing")} style={{ all: "unset", cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
          <Logo size={32} /><span style={{ font: "800 17px Inter", color: "#F8F5F0", letterSpacing: ".02em" }}>AFROLOC</span>
        </button>
        <div style={{ display: "flex", gap: 4, marginLeft: 10 }}>
          {TOP.map((n) => (
            <button key={n.label} onClick={() => navigate("/" + n.id)} style={{ all: "unset", cursor: "pointer", font: `${topActive === n.id ? 700 : 500} 13px Inter`, color: topActive === n.id ? "#E8C97A" : "#A99E8C", padding: "6px 11px" }}>{n.label}</button>
          ))}
        </div>
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
        <div style={{ width: 220, flex: "none", background: "#F0EADE", borderRight: "1px solid #E0D6C6", padding: "20px 14px", display: "flex", flexDirection: "column", gap: 3 }}>
          <div style={{ font: "700 10px Inter", letterSpacing: ".14em", color: "#B0831F", textTransform: "uppercase", padding: "0 13px 9px" }}>Documentos</div>
          {SIDE.map((s) => {
            const on = side === s.id;
            return (
              <button key={s.id} onClick={() => navigate("/" + s.id)} style={{ all: "unset", cursor: "pointer", padding: "9px 13px", borderRadius: 9, font: `${on ? 700 : 500} 13px Inter`, color: on ? "#1A1814" : "#8A8073", background: on ? "#F0E5CE" : "transparent" }}>{s.label}</button>
            );
          })}
        </div>
